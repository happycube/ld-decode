	...
	public:
		TBC(int _bufsize = 4096) {
			...
			f_q = new Filter(15, NULL, f14_1_3mhz_b15);
                        ...
                        f_synci = new Filter(30, NULL, f14_0_6mhz_b30);
                        f_syncq = new Filter(30, NULL, f14_0_6mhz_b30);
		}
...
/*			    (sync2_len > (16 * freq)) && (sync2_len < (18 * freq)) && */
			    (sync_len > (15 * freq)) && (sync_len < (20 * freq)))
			{
				...
				BurstDetect(&buf[sync_start], 2.5 * dots_usec, 4.5 * dots_usec, plevel, pphase);
				cerr << curline << " start " << sync_start << " burst 1 " << plevel << " " << pphase << endl;
		
				// cerr << sync_len << ' ' << (sync2_start - sync_start + sync2_len) << endl;	
				BurstDetect(&buf[sync_start], (sync2_start - sync_start) + (2.5 * dots_usec), 4.5 * dots_usec, plevel2, pphase2);
				...
				if ((plevel > 500) && (plevel2 > 500)) {
					...
					BurstDetect(buf, 2.5 * dots_usec, 4.5 * dots_usec, plevel, pphase);
