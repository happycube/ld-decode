...
Pll::Pll()
{
    ...
    minimumPeriod  = basePeriod / 1.05; // -5% deviation from base period
    maximumPeriod  = basePeriod * 1.05; // +5% deviation from base period
    periodAdjustBase = basePeriod / 10000.0; // Clock adjustment of 10,000 steps between min and max period

    phaseAdjustMultiplier = 0.005; // 0.005
    ...
    qDebug() << "Pll::Pll(): Base period is" << basePeriod;
    qDebug() << "Pll::Pll(): Minimum period is" << minimumPeriod;
    qDebug() << "Pll::Pll(): Maximum period is" << maximumPeriod;
    qDebug() << "Pll::Pll(): Period adjust base is" << periodAdjustBase;
}
...
void Pll::pushEdge(qreal sampleDelta)
{
    while(sampleDelta >= refClockTime) {
        ...
        if((sampleDelta > next || tCounter < 3) && !(tCounter > 10)) {
            ...
            phaseAdjust = delta * phaseAdjustMultiplier;
