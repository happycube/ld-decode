...
// Reset statistics
void F1ToSectors::resetStatistics(void)
{
    validSectors = 0;
    invalidSectors = 0;
    correctedSectors = 0;
...
QVector<Sector> F1ToSectors::convert(QVector<F1Frame> f1FramesIn)
{
    ...
    for (qint32 i = 0; i < f1FramesIn.size(); i++) {
        ...
        if (sector.isValid()) {
            ...
            //qDebug() << "F1ToSectors::convert(): F1Frame mode =" << sector.getMode() << "address =" << sector.getAddress().getTimeAsQString();
        } else {
            invalidSectors++;

            qDebug() << "F1ToSectors::convert(): F1Frame mode =" << sector.getMode() << "address =" << sector.getAddress().getTimeAsQString() << "Invalid";
        }
