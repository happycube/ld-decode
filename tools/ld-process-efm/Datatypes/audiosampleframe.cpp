...
#include "audiosampleframe.h"

AudioSampleFrame::AudioSampleFrame()
{
    // Set the default values for the audio sample
    sample.metadata.discTime.setTime(0, 0, 0);
    sample.metadata.trackTime.setTime(0, 0, 0);
    sample.metadata.trackNumber = -1;

    silenceSample();
}

// Method to set the audio sample frame data from a F2 Frame
AudioSampleFrame::AudioSampleFrame(F2Frame f2Frame)
{
    setDataFromF2Frame(f2Frame);
}
...
void AudioSampleFrame::setDataFromF2Frame(F2Frame f2Frame)
{
    ...
    // Set the sample type (based on the F2 metadata)
    if (!f2Frame.getIsEncoderRunning()) sample.metadata.sampleType = SampleType::silence;
    else if (f2Frame.isFrameCorrupt()) sample.metadata.sampleType = SampleType::corrupt;
    else sample.metadata.sampleType = SampleType::audio;

    // Populate the sample values from the frame
    createSampleValuesFromFrame();
}

// Method to get the audio sample metadata
AudioSampleFrame::Metadata AudioSampleFrame::getMetadata()
{
    return sample.metadata;
}

// Method to set the audio sample metadata
void AudioSampleFrame::setMetadata(Metadata _metadata)
{
...
void AudioSampleFrame::setSampleValues(AudioSampleFrame::SampleValues _sampleValues)
{
...
AudioSampleFrame::SampleValues AudioSampleFrame::getSampleValues()
{
    return sample.sampleValues;
}

// Method to set the signed 16-bit sample values to silence
void AudioSampleFrame::setSampleToSilence()
{
    silenceSample();
}
...
void AudioSampleFrame::createSampleValuesFromFrame()
{
    // Set the sample data
...
void AudioSampleFrame::silenceSample()
{
