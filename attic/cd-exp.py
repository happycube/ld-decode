...
N, Wn = sps.buttord([(left_audfreq-.05) / hafreq, (left_audfreq+.05) / hafreq], [(left_audfreq-.15) / hafreq, (left_audfreq+.15)/hafreq], 1, 20)
leftbp_filter_b, leftbp_filter_a = sps.butter(N, Wn, btype='bandpass')

N, Wn = sps.buttord([(right_audfreq-.05) / hafreq, (right_audfreq+.05) / hafreq], [(right_audfreq-.15) / hafreq, (right_audfreq+.15)/hafreq], 1, 20)
...
N, Wn = sps.buttord(0.016 / hafreq, 0.024 / hafreq, 1, 5) 
...
# CLD-900 service guide says a 1.75mhz lpf is used 	
efm_filter_b, efm_filter_a = sps.butter(10, (1.75/(freq/2)), 'low')
efm_filter_b, efm_filter_a = sps.butter(14, (1.75/(freq/2)), 'low')
#doplot(efm_filter_b, efm_filter_a)
#exit()

efm_filter_bf = sps.firwin(65, [.25/(freq/2), 1.90/(freq/2)], pass_zero=False)
efm_filter_bf = sps.firwin(65, [.2/(freq/2), 0.65/(freq/2)], pass_zero=False)
efm_filter_bf = sps.firwin(49, [.05/(freq/2), 1.10/(freq/2)], pass_zero=False)
#efm_filter_bf = sps.firwin(65, [.3/(freq/2), 1.05/(freq/2)], pass_zero=False)
efm_filter_af = [1.0]
#doplot2(efm_filter_b, efm_filter_a, efm_filter_bf, efm_filter_af)
#dosplot(efm_filter_bf, efm_filter_af)
#exit()

avf_filter_b, avf_filter_a = sps.butter(3, 0.005)

hfreq = freq / 2
N, Wn = sps.buttord([0.30 / hfreq, 0.6 / hfreq], [0.20 / hfreq, 0.70/hfreq], 2, 10)
print N, Wn
#efm_filter_bf, efm_filter_af = sps.butter(N, Wn, btype='bandpass')

N, Wn = sps.buttord([0.15 / hfreq, 0.85 / hfreq], [0.03 / hfreq, 1.70/hfreq], 1, 22)
print N, Wn
#efm_filter_bf, efm_filter_af = sps.butter(N, Wn, btype='bandpass')
#efm_filter_bf, efm_filter_af = fdls.FDLS_fromfilt(efm_filter_bf, efm_filter_af, 61, 0)

dosplot(efm_filter_bf, efm_filter_af)
#doplot2(efm_filter_bf, efm_filter_af, efm_filter_bf2, efm_filter_af2)
#exit()

def process_audio(indata):
	x = 108 
	in_filt = indata[x:x+8000]
#	plt.plot(range(0, len(in_filt)), in_filt, 'b')
	m = np.mean(in_filt)
	x = np.empty(len(in_filt))
	x[in_filt > m] = 128
	x[in_filt <= m] = 0
#	plt.plot(range(0, len(x)), x, 'b')

	in_filt = sps.lfilter(efm_filter_bf, efm_filter_af, indata)[128:]
	in_filt = in_filt[0:4000]

	in_afilt = sps.lfilter(avf_filter_b, avf_filter_a, in_filt)[128:]

	in_filt = in_filt[500:]
	in_afilt = in_afilt[500:]

	#plt.plot(range(0, len(in_filt)), in_filt, 'r')
#	plt.plot(range(0, len(in_afilt)), in_afilt, 'b')
#	plt.show()
#	exit()
	...
	prevt = 0
	prevp = 0
	for i in range(6, len(in_filt) - 6):
		if (((in_filt[i] > in_filt[i - 6]) and (in_filt[i] > in_filt[i + 6])) or ((in_filt[i] < in_filt[i - 6]) and (in_filt[i] < in_filt[i + 6]))) and ((i - prevp) > 14):
			if (((in_filt[i] > in_filt[i - 1]) and (in_filt[i] > in_filt[i + 1])) or ((in_filt[i] < in_filt[i - 1]) and (in_filt[i] < in_filt[i + 1]))):
				print "P", i, prevp, i - prevp, in_filt[i - 1], in_filt[i], in_filt[i + 1]
				prevp = i

		if x[i] != prev:
			prev = x[i]
#			print "Z", i, prevt, i - prevt 
			prevt = i

#	plt.plot(range(0, len(x)), x, 'g')
	
#	in_filt = sps.lfilter(efm_filter_bf, efm_filter_af, indata)[128:]
#	in_filt = in_filt[0:8000] - np.mean(in_filt[0:8000])
