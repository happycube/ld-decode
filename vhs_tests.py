...
class DemodTest(unittest.TestCase):
    def test_ire(self):
        ...
        samplerate_mhz = 40
        decoder = vhs_process.VHSRFDecode(inputfreq=samplerate_mhz,system='PAL')
        max_hz = 4800000
        max_mhz = max_hz / 1000000
        min_hz = 3800000
        min_mhz = min_hz / 1000000
        wavemax = vhs_utils.gen_wave_at_frequency(max_mhz, samplerate_mhz, decoder.blocklen / 2)
        wavemin = vhs_utils.gen_wave_at_frequency(min_mhz, samplerate_mhz, decoder.blocklen / 2)
