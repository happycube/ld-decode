...
ld-chroma-decoder -f ntsc2d --ntsc-phase-comp --input-json $1.tbc.json $1_chroma_doc.tbc $1_chroma.rgb

ffmpeg -f rawvideo -r 30000/1001 -pix_fmt rgb48 -s 760x488 -i $1_chroma.rgb -r 30000/1001 -pix_fmt rgb48 -s 760x488 -i $1.rgb -filter_complex "[0:v]format=yuv444p16le[chroma];[1:v]format=yuv444p16le[luma];[chroma][luma]mergeplanes=0x100102:yuv444p16le[output]" -map "[output]":v -c:v libx264 -qp 0 -pix_fmt yuv444p16le -top 1 -color_range tv -color_primaries smpte170m -color_trc smpte170m -colorspace smpte170m -aspect 4:3 -y $1.mkv
