...
def quadpeak(y):
	return ((y[2] - y[0]) / (2 * (2 * y[1] - y[0] - y[2])) * 2)

def find_peaks1(data):
	prev = -1 
	for i in range(1, 5800):
	#for i in range(1, len(data)-1):
		#if ((data[i] > data[i - 1]) and (data[i] > data[i + 1])) or (data[i] < data[i - 1]) and (data[i] < data[i + 1]):
		if data[i] > 4 and ((data[i] > data[i - 1]) and (data[i] > data[i + 1])):
			qi =  i + quadpeak(data[i-1:i+2])
			rqi = np.round((qi - prev) / 6.626)
			#print(i, qi, (qi - prev) / 6.626, data[i - 1], data[i], data[i + 1])
			print(i, qi, rqi, (qi - prev) / 6.626, data[i - 1], data[i], data[i + 1])
			if (prev > 0):
				prev = prev + (6.626 * rqi)
			else:
				prev = qi
...
def find_zc1(data):
	...
	prev = 0
	for i in zero_crossings3:
		print(i, (i - prev) / 6.626, data[i - 1], data[i], data[i + 1], data[i - 1] - data[i + 1])
		prev = i
...
def find_zc(data):
	...
	#for i in range(1, len(data)-1):
		if ((data[i] <= 0) and (data[i - 1] > 0)) or ((data[i] > 0) and (data[i - 1] <= 0)):
			#print(i, (i - prev) / 6.26, data[i - 1], data[i], data[i + 1])
			qi = i + np.abs((data[i] - data[i - 1]) / (data[i + 1] - (data[i - 1])))
			print(i, qi, (qi - prev) / 6.626, data[i - 1], data[i], data[i + 1], data[i] - data[i - 1], data[i - 1] - data[i + 1])
			prev = i
