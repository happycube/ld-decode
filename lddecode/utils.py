...
if not numba.version_info.major and numba.version_info.minor < 59:
    ...
    @njit(cache=True,nogil=True)
    def unwrap_hilbert(hilbert, freq_hz):
        """FM demodulate via hilbert phase unwrapping.

        Fused single-pass: computes atan2, angle diffs, unwrap, clamp,
        and frequency scaling in one loop over the data, eliminating
        intermediate array allocations and multiple passes.
        """
        n = len(hilbert)
        output = np.empty(n, dtype=np.float64)
        scale = freq_hz / tau

        # Element 0: dangle is 0 by definition (to_begin=0), clamped to [0,tau) → 0
        prev_angle = math.atan2(hilbert[0].imag, hilbert[0].real)
        output[0] = 0.0

        prev_dangle = 0.0  # previous raw dangle (for unwrap delta tracking)
        correction = 0.0   # cumulative unwrap correction

        for i in range(1, n):
            cur_angle = math.atan2(hilbert[i].imag, hilbert[i].real)
            dangle = cur_angle - prev_angle
            prev_angle = cur_angle

            # Unwrap: keep consecutive dangles within pi of each other
            dd = dangle - prev_dangle
            prev_dangle = dangle
            ...
            # Apply correction and clamp to [0, tau)
            val = dangle + correction
            val = val - math.floor(val / tau) * tau
...
def findpulses(sync_ref, _, high):
    """Locate possible pulses by looking at areas within some range.
    Returns (pulses_list, starts_array, lengths_array).
    """
...
@njit(cache=True, nogil=True)
def compute_all_burst_offsets_nb(
    demod_burst, demod, linelocs, lineoffset, linecount,
    samplesperline, linelen, fsc_mhz, hz_ire_30,
    prev_phaseadjust
):
    """Batched burst offset computation for all 266 lines.

    Replaces the Python loop in compute_burst_offsets + compute_line_bursts
    to avoid ~1600 Python-to-numba boundary crossings per field.

    Returns (adjs, rising_sum) where adjs is float64[266] with NaN for
    lines where no valid burst was found.
    """
    fsc_mhz_inv = 1.0 / fsc_mhz
    bstime = 21.0 * fsc_mhz_inv
    betime = 28.0 * fsc_mhz_inv

    adjs = np.full(266, np.nan)
    rising_sum = 0

    isrising = np.zeros(16, dtype=np.bool_)
    zcs = np.zeros(16, dtype=np.float32)

    for _line in range(266):
        line = _line + lineoffset

        # inline get_linelen
        if line >= linecount + lineoffset:
            length = linelocs[line] - linelocs[line - 1]
        elif line > 0:
            length = (linelocs[line + 1] - linelocs[line - 1]) / 2.0
        elif line == 0:
            length = linelocs[line + 1] - linelocs[line]
        else:
            length = linelen
        ...
        burstarea = demod_burst[s + bstart : s + bend]
        if len(burstarea) == 0:
            continue
        ...
        burstarea_demod = demod[s + bstart : s + bend]
        burstarea_demod = burstarea_demod - nb_mean(burstarea_demod)

        if nb_absmax(burstarea_demod) > hz_ire_30:
            continue

        zcburstdiv = (lfreq * fsc_mhz_inv) / 2.0
        phase_adjust = -prev_phaseadjust

        for passcount in range(2):
            zc_count, phase_adjust, rising_count = clb_findbursts(
                isrising, zcs, burstarea, 0,
                len(burstarea) - 1, threshold,
                bstart, s_rem, zcburstdiv, phase_adjust
            )

        if zc_count > 0:
            rising = rising_count > (zc_count / 2)
            adjs[_line] = (-phase_adjust) / 2.0

            if not (_line % 2) and rising:
                rising_sum += 1

    return adjs, rising_sum


@njit(cache=True, nogil=True)
def refine_linelocs_burst_nb(
    demod_burst, demod, linelocs_in, linebad, lineoffset, linecount,
    samplesperline, linelen, fsc_mhz, hz_ire_30,
    prev_phaseadjust, n_lines,
):
    """Combined burst offset computation + line location refinement.

    Merges compute_all_burst_offsets_nb + refine_linelocs_burst into one
    numba call, eliminating dict conversions and repeated get_linefreq calls.

    Returns (linelocs_adj, field14, phase_adjust_median).
    Modifies linebad in-place.
    """
    fsc_mhz_inv = 1.0 / fsc_mhz
    bstime = 21.0 * fsc_mhz_inv
    betime = 28.0 * fsc_mhz_inv

    # --- Phase 1: compute burst offsets (same as compute_all_burst_offsets_nb) ---
    raw_adjs = np.full(n_lines, np.nan)
    rising_sum = 0
    ...
    for _line in range(n_lines):
        ...
        # inline get_linelen
        if line >= linecount + lineoffset:
            length = linelocs_in[line] - linelocs_in[line - 1]
        elif line > 0:
            length = (linelocs_in[line + 1] - linelocs_in[line - 1]) / 2.0
        elif line == 0:
            length = linelocs_in[line + 1] - linelocs_in[line]
        else:
            length = linelen
        ...
        s = int(linelocs_in[line])
        s_rem = linelocs_in[line] - s
        ...
        if zc_count > 0:
            rising = rising_count > (zc_count / 2)
            raw_adjs[_line] = (-phase_adjust) / 2.0

            if not (_line % 2) and rising:
                rising_sum += 1

    # --- Phase 2: mark lines without bursts as bad ---
    for l in range(1, n_lines):
        if np.isnan(raw_adjs[l]):
            linebad[l] = True

    # --- Phase 3: scale adjustments by lfreq * fsc_mhz_inv ---
    scaled_adjs = np.full(n_lines, np.nan)
    for l in range(n_lines):
        if np.isnan(linelocs_in[l]) or linebad[l] or np.isnan(raw_adjs[l]):
            continue

        # inline get_linefreq (same logic as above)
        line = l + lineoffset
        if line >= linecount + lineoffset:
            length = linelocs_in[line] - linelocs_in[line - 1]
        elif line > 0:
        ...
        lfreq = samplesperline * length
        scaled_adjs[l] = raw_adjs[l] * lfreq * fsc_mhz_inv

    # --- Phase 4: compute median of valid scaled adjustments ---
    valid_count = 0
    for l in range(n_lines):
        if not np.isnan(scaled_adjs[l]):
            valid_count += 1

    linelocs_adj = linelocs_in.copy()

    if valid_count == 0:
        # field14 = rising_sum > 0 (len(adjs)//4 with 0 adjs = 0)
        return linelocs_adj, rising_sum > 0, 0.0

    valid_vals = np.empty(valid_count, dtype=np.float64)
    idx = 0
    for l in range(n_lines):
        if not np.isnan(scaled_adjs[l]):
            valid_vals[idx] = scaled_adjs[l]
            idx += 1

    adjs_median = np.median(valid_vals)

    # Compute phase_adjust_median (= median of raw_adjs * 2)
    raw_valid = np.empty(valid_count, dtype=np.float64)
    idx = 0
    for l in range(n_lines):
        if not np.isnan(raw_adjs[l]):
            raw_valid[idx] = raw_adjs[l]
            idx += 1
    phase_adjust_median = np.median(raw_valid) * 2.0

    # --- Phase 5: apply adjustments with outlier rejection ---
    lastvalid_adj = adjs_median
    for l in range(n_lines):
        if not np.isnan(scaled_adjs[l]) and np.abs(scaled_adjs[l] - adjs_median) <= 2.0:
            linelocs_adj[l] += scaled_adjs[l]
            lastvalid_adj = scaled_adjs[l]
        else:
            linelocs_adj[l] += lastvalid_adj

    field14 = rising_sum > (valid_count // 4)

    return linelocs_adj, field14, phase_adjust_median


@njit(cache=True, nogil=True)
def calc_burstmedian_nb(demod, linelocs, lineoffset, linecount,
                        samplesperline, linelen, begin_usec, length_usec,
                        line_start, line_end, burst_threshold):
    """Batch compute burst levels for a range of lines and return median.

    Works for both NTSC and PAL:
    - NTSC: line_start=11, line_end=264, burst_threshold=0.0 (no filtering)
    - PAL:  line_start=11, line_end=313, burst_threshold>0 (filter high bursts)

    When burst_threshold > 0, mean is subtracted and lines where
    max(burstarea) > burst_threshold are skipped (PAL behavior).
    """
    max_lines = line_end - line_start
    burstlevels = np.empty(max_lines, dtype=np.float64)
    count = 0
    sqrt2 = np.sqrt(2.0)
    do_threshold = burst_threshold > 0.0

    for _line in range(line_start, line_end):
        line = _line + lineoffset
        ...
        begin = linelocs[line] + begin_usec * lfreq
        end = begin + length_usec * lfreq + 1

        burstarea = demod[int(begin):int(end)]
        if len(burstarea) == 0:
            continue

        if do_threshold:
            # PAL path: subtract mean, check threshold
            mean_val = np.mean(burstarea)
            adjusted = burstarea - mean_val
            if np.max(adjusted) > burst_threshold:
                continue
            burstlevels[count] = rms(adjusted) * sqrt2
        else:
            # NTSC path: direct rms
            burstlevels[count] = rms(burstarea) * sqrt2
    ...
    return np.median(burstlevels[:count])


@njit(cache=True, nogil=True)
def refine_linelocs_hsync_nb(demod_05, linelocs1, linebad, freq,
                              target_zc1, hz_m55, hz_30, is_pal):
    """Batch refine line locations via hsync zero-crossing detection.

    Replaces the Python loop in Field.refine_linelocs_hsync to avoid
    ~1300 Python-to-numba boundary crossings per field.
    """
    n = len(linelocs1)
    ...
    for i in range(n):
        ...
        # first calczc: find zero crossing
        ll1 = linelocs1[i] - freq
        zc = calczc_do(demod_05, ll1, target_zc1, 0, count_zc1)

        if zc is not None and not linebad[i]:
            linelocs2[i] = zc

            # check hsync area bounds
            area_start = int(zc - (freq * 0.75))
            area_end = int(zc + (freq * 8))
            hsync_area = demod_05[area_start:area_end]

            if nb_min(hsync_area) < hz_m55 or nb_max(hsync_area) > hz_30:
                linebad[i] = True
                linelocs2[i] = linelocs1[i]
            else:
                # compute porch and sync levels
                porch_level = nb_median(
                    demod_05[int(zc + (freq * 8)):int(zc + (freq * 9))]
                )
                sync_level = nb_median(
                    demod_05[int(zc + freq):int(zc + (freq * 2.5))]
                )

                # second calczc with refined target
                zc2 = calczc_do(demod_05, ll1,
                                (porch_level + sync_level) / 2.0, 0, 400)

                if zc2 is not None and np.abs(zc2 - zc) < freq_half:
                    linelocs2[i] = zc2
                else:
                    linebad[i] = True
        else:
