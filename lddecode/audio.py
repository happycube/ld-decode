...
class AudioFMDemod:
    def __init__(self, SysParams, center_freq):
        self.SysParams = SysParams
        self.center_freq = center_freq

        blocklen = SysParams['blocklen']
        blockskip = SysParams['blockskip']
        apass = SysParams['audio_filterwidth']
        
        freq_hz = SysParams['freq_hz']
        freq_hz_half = freq_hz / 2

        # Compute stage 1 filters
        audio1_fir = utils.filtfft([sps.firwin(SysParams['afilt_len'], [(self.center_freq-apass)/freq_hz_half, (self.center_freq+apass)/freq_hz_half], pass_zero=False), 1.0], blocklen)
        ...
        self.low_freq = SysParams['freq_hz'] * (lowbin / blocklen)
    ...
    def process_stage2(self):
        ...
        filtered = utils.sqsum(a2[self.SysParams['blocklen_dropb']:-self.SysParams['blocklen_drope']])
...
class Audio:
    def __init__(self, args):
        ...
        SysParams['freq_hz_half'] = SysParams['freq_hz'] / 2
        ...
        self.blocklen = SysParams['blocklen']
        self.blockskip = SysParams['blocklen_dropb'] + SysParams['blocklen_drope']
        SysParams['blockskip'] = self.blockskip
        self.dropb = SysParams['blocklen_dropb']
        self.drope = SysParams['blocklen_drope']

        apass = SysParams['audio_filterwidth']
        SysParams['afilt_len'] = 512

        self.aa_channels = []

        if True:
            self.aa_channels.append(AudioFMDemod(SysParams, SysParams['audio_lfreq']))
            
        if True:
            self.aa_channels.append(AudioFMDemod(SysParams, SysParams['audio_rfreq']))
        ...
        self.efm_decode = not args.noefm
        if self.efm_decode:
            self.efm_filter = efm_pll.computeefmfilter(SysParams['freq_hz'], self.blocklen)
    ...
    def process_data(self, rawdata):
        ...
        if len(rawdata) != self.blocklen:
            # TODO: throw an exception
            print('err')
            return None

        fft_in = npfft.fft(rawdata)
        ...
        if self.efm_decode:
            filtered_efm = npfft.ifft(fft_in * self.efm_filter)[self.dropb:-self.drope]
...
input_buffer = utils.StridedCollector(audio.blocklen*2, audio.blockskip*2)
