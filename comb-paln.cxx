...
struct YIQ {
        ...
        YIQ(double _y = 0.0, double _i = 0.0, double _q = 0.0, double _u = 0.0, double _v = 0.0) {
                y = _y; i = _i; q = _q; u = _u; v = _v;
        };
...
struct frame_t {
	...
	double burstangle[in_y];

	double buffery[in_y][in_x];
	double bufferu[4][in_y][in_x];
	double bufferv[4][in_y][in_x];
};
...
#endif
		// Hacked in dimension selection to support PAL 
		void SplitIQ(int f, int d = -1) {
			double mse = 0.0;
			double me = 0.0;
...
//				if (f_neuralnet) invertphase = true;
				...
				for (int h = 4; h < (in_x - 4); h++) {
					...
					double k2 = Frame[f].combk[2][l][h];
					double k1 = Frame[f].combk[1][l][h];
					double k0 = Frame[f].combk[0][l][h];

					if (d == 2) {k2 = 1; k1 = 0; k0 = 0;}
					if (d == 1) {k2 = 0; k1 = 1; k0 = 0;}
					if (d == 0) {k2 = 0; k1 = 0; k0 = 1;}

					cavg += (Frame[f].clpbuffer[2][l][h] * k2);
					cavg += (Frame[f].clpbuffer[1][l][h] * k1);
					cavg += (Frame[f].clpbuffer[0][l][h] * k0);
		...
		void MergeK(int f) {
			...
			for (int l = 24; l < in_y; l++) {
				...
				for (int h = 4; h < (in_x - 4); h++) {
					...
					uavg += (Frame[f].bufferu[2][l][h] * k2);
					uavg += (Frame[f].bufferu[1][l][h] * k1);
					uavg += (Frame[f].bufferu[0][l][h] * k0);
					uavg /= 2;
					
					vavg += (Frame[f].bufferu[2][l][h] * k2);
					vavg += (Frame[f].bufferu[1][l][h] * k1);
					vavg += (Frame[f].bufferu[0][l][h] * k0);
					vavg /= 2;

					Frame[f].cbuf[l].p[h].y = line[h]; 
					Frame[f].cbuf[l].p[h].i = uavg;  
					Frame[f].cbuf[l].p[h].q = vavg; 
					Frame[f].cbuf[l].p[h].u = uavg;  
					Frame[f].cbuf[l].p[h].v = vavg; 
					
					if (f_bw) {
						Frame[f].cbuf[l].p[h].i = Frame[f].cbuf[l].p[h].q = 0;  
						Frame[f].cbuf[l].p[h].u = Frame[f].cbuf[l].p[h].v = 0;  
					}
				}
			}
		}
...
//					if (l == (f_debugline + lineoffset)) cerr << a << ' ' << input->p[h].y << endl; 
		...
		// This phase-related stuff is hackish but (usually?) works.  Run this routine after 1D
		void GetPALPhase(int f) {
			SplitIQ(f, 0);
...
//					cerr << "BURST " << l << ' ' << h << ' ' << yiq.y << ' ' << yiq.i << ' ' << yiq.q << ' ' << ctor(yiq.i, yiq.q) << ' ' << atan2deg(yiq.q, yiq.i) << endl;
				...
				Frame[f].burstangle[l] = atan2deg(q, i);
...
//				cerr << "angle of " << l << " is " << angle[l] << ' ' <<  endl; 
			...
			for (int l = 20; l < (in_y - 4); l += 4, tot++) {
				if (fabs(Frame[f].burstangle[l + 1] - Frame[f].burstangle[l]) < 20) phasecount++; 
			}
			Frame[f].phase = phasecount > (tot / 2);
		}

		// clpbuffer => bufferu and bufferv
		void CtoUV(int f, int dim) {
			SplitIQ(f, dim);

			for (int l = 10; l < in_y; l++) {
				double burstlev = 8; // Frame[f].rawbuffer[(l * in_x) + 1] / irescale;
				uint16_t *line_output = &output[(in_x * 3 * (l - 10))];
				int o = 0;
				...
				double angleadj = 135 - Frame[f].burstangle[l];
	
				for (int h = 0; h < in_x; h++) {
					double adj2 = 0;

					double i = +(Frame[f].cbuf[l].p[h].i);
					double q = +(Frame[f].cbuf[l].p[h].q);
					...
					if (l == (f_debugline + lineoffset))
						cerr << "A " << h << ' ' << Frame[f].cbuf[l].p[h].i << ' ' << Frame[f].cbuf[l].p[h].q << ' ' ;

					Frame[f].bufferu[dim][l][h] = cos(angle) * mag;
					Frame[f].bufferv[dim][l][h] = sin(angle) * mag;

					if (l == (f_debugline + lineoffset)) 
						cerr << Frame[f].bufferu[dim][l][h] << ' ' << Frame[f].bufferv[dim][l][h] << endl ;
				}
			}
		}
