...
//						cerr << cfline << ' ' << i << ' ' << firstone << ' ' << bit * 57 << ' ' << bit << ' ' << hex << code << dec << endl;
		...
		NTSColor(vector<YIQ> *_buf = NULL, LDE *_f_post = NULL, LDE *_f_postc = NULL, double _freq = 8.0) {
			...
			f_synci = new LDE(45, NULL, f28_0_6mhz_b45);
			f_syncq = new LDE(45, NULL, f28_0_6mhz_b45);
			...
			if (insync == false) {
				...
				if ((igap > 1000) && lastsync == 200) {
					...
					if ((level > .03) && (level < .18)) {
						...
						adjfreq = 1820.0 / (1820 + (padj * 1.18 * (M_PIl / 2.0)));
					}
...
//					if (buf && lastsync == 200 && igap >= 0) cerr << "S " << counter << ' ' << igap << endl;
			...
			if ((lastsync > 100) && (lastsync < 220)) { 
				f_syncq->feed(in * _cos[counter % 8]);
				f_synci->feed(-in * _sin[counter % 8]);
			}
