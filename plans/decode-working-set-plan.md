...
*Done:* [`decode-throughput-plan.md`](decode-throughput-plan.md) §8, on the Phase 5 code. Both
curves knee at 4 — `-t 4` in all three cells, N = 4 for concurrent serial decoders — and the hot
set is 5.60 MiB against Phase 0's 11.5, so 32 MiB of L3 holds 5.7 of them where it held 2.8.
...
*Done:* the plateau's onset is at 4, half the box's physical core count, so by the letter of this
task Phases 5–7 are not closed out. They are nonetheless retired, for a reason this task did not
anticipate: the plateau is no longer a footprint effect that a smaller decoder lifts. At the knee
the tree moves 7.6 GB/s of a 20 GB/s path and 5.6 of 8 cores are busy; what stops a fifth decoder
paying is traffic *per frame* generated by concurrency itself, at constant instructions. The
remaining lever is bytes streamed per frame, which
[`decode-throughput-plan.md`](decode-throughput-plan.md) §8 Task 5 hands to a separate plan.
...
*Open, and now evidenced.* `main.py:504` gives 6 on the reference box; the measured optimum is 4 in
every cell. At 6, PAL CVBS and NTSC are unchanged within the run-to-run spread and PAL `--tbc`
loses 6.3%, for 630 MB more peak tree RSS. The figures are in
[`decode-throughput-plan.md`](decode-throughput-plan.md) §8 Task 1; the change itself is not made
there, because that phase changes no code.
