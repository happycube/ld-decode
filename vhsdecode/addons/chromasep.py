...
class ChromaSepClass:
    ...
    def work(self, luminance):
        downsampled = self.method(
            luminance, self.ratio.numerator, self.ratio.denominator, converter_type=self.converter_type
        )
        ...
        result = self.method(
            combed, self.ratio.denominator, self.ratio.numerator, converter_type=self.converter_type
        )
