...
if [ -f "$1.wav" ] ; then
  ffmpeg -hide_banner -thread_queue_size 4096 -f rawvideo -r 30000/1001 -pixel_format gray16le -s 760x488 \
  -color_range tv \
  -i <( \
  ...
  -i <( \
    ...
    ld-chroma-decoder -f $CHROMA_DECODER --chroma-gain $CHROMA_GAIN -p yuv --input-json $1.tbc.json - - \
  ) \
